...
class Seller:
    ...
    def get_revenues(self):
        fee = 0.1
        fee_2 = 80
        rev_results = fee * self.get_sales()['sales'] + fee_2 * self.get_active_dates()['months_on_olist']
        rev_results = pd.DataFrame(rev_results).reset_index()
        rev_results.columns = ['seller_id', 'revenues']
        return rev_results


    def get_training_data(self, revenue='off'):
        """
        Returns a DataFrame with:
        ...
        if revenue == 'off':
            ...
            training_set_updated = training_set.merge(self.get_revenues(),
                                                      on='seller_id')
